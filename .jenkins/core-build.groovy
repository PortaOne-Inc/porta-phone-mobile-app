...
//
// Job parameters:
//   SRC_REF          - branch, tag or commit of porta-phone/backend to build. A core/vX.Y.Z tag
//                      produces :X.Y.Z (checked against @version in core/mix.exs); anything else
//                      produces <mix version>-<short sha>.
//   PIPELINE_REF     - which ref of THIS repository the job reads this script from.
//   PIPELINE_REFSPEC - fetch refspec for that ref, so an unmerged Gerrit patchset
//                      (refs/changes/NN/CCCCCC/P) can be tested before it is merged.
