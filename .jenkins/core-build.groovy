...
//
// The job also builds itself when a core/v* tag is pushed to porta-phone/backend: the
// Gerrit ref-updated event supplies GERRIT_REFNAME, and any other updated ref is ignored.
...
pipeline {
  ...
  stages {
      ...
      steps {
        script {
          ...
          if (env.GERRIT_EVENT_TYPE == 'ref-updated' && env.GERRIT_REFNAME) {
            env.SRC = env.GERRIT_REFNAME
            env.BUILD_IT = env.SRC.startsWith('refs/tags/core/v') ? 'true' : 'false'
            if (env.BUILD_IT != 'true') {
              currentBuild.result = 'NOT_BUILT'
              currentBuild.description = "ignored ${env.SRC}"
              echo "Ref ${env.SRC} is not a core release tag - nothing to build."
              return
            }
            env.SRC = env.SRC.replaceFirst('^refs/tags/', '')
          } else {
    ...
    stage('Resolve version') {
      when { expression { env.BUILD_IT == 'true' } }
      steps {
        script {
