...
class _PublicationScreenState extends State<PublicationScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return AuthReloginTrigger(
      onRelogin: () => context.read<PublicationResourcesCubit>().load(),
      child: MultiBlocListener(
        listeners: [
          BlocListener<PublicationResourcesCubit, PublicationResourcesState>(
            ...
            listener: (_, s) => _snack(context, 'Create failed: ${s.createError}'),
          ),
          BlocListener<PublicationResourcesCubit, PublicationResourcesState>(
            listenWhen: (p, n) => p.updateError != n.updateError && n.updateError != null,
            listener: (_, s) => _snack(context, 'Update failed: ${s.updateError}'),
          ),
          BlocListener<PublicationResourcesCubit, PublicationResourcesState>(
            listenWhen: (p, n) => p.deleteError != n.deleteError && n.deleteError != null,
            listener: (_, s) => _snack(context, 'Delete failed: ${s.deleteError}'),
          ),
        ],
              ...
              appBar: AppBar(
                ...
                actions: [
                  IconButton(
                    ...
                    onPressed: () => context.read<PublicationResourcesCubit>().load(),
                  ),
                ],
              ),
    ...
    switch (state.status) {
      ...
      case PubResStatus.loaded:
        if (state.items.isEmpty) {
          return _EmptyView(onRefresh: () => context.read<PublicationResourcesCubit>().load());
        }
        return ListView.separated(
          ...
          itemBuilder: (context, i) {
            ...
            return ListTile(
              ...
              title: Text(item.title?.isNotEmpty ?? true ? item.title! : (item.url ?? 'Untitled')),
              subtitle: Text(
                [
                  if (item.url?.isNotEmpty ?? true) 'url: ${item.url}',
                  if (item.note?.isNotEmpty ?? true) 'note: ${item.note}',
                  if (item.text?.isNotEmpty ?? true) 'text: ${_short(item.text!, 60)}',
                ].join(' • '),
                maxLines: 2,
              ...
              trailing: Wrap(
                ...
                children: [
                    ...
                    onPressed: isUpdating
                        ...
                        : () async {
                            ...
                            if (result != null && context.mounted) {
                              await context.read<PublicationResourcesCubit>().update(
                                id,
                                title: result.title,
                                url: result.url,
                                note: result.note,
                  ...
                  IconButton(
                    tooltip: 'Delete',
                    onPressed: isDeleting ? null : () => context.read<PublicationResourcesCubit>().delete(id),
                    icon: isDeleting
                        ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2))
  ...
  Future<void> _openCreateDialog(BuildContext context) async {
    final result = await showDialog<_EditorResult>(context: context, builder: (_) => const _EditorDialog());
  ...
  String _short(String s, int max) => s.length <= max ? s : '${s.substring(0, max)}…';
}
...
class _EditorDialogState extends State<_EditorDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      title: Text(isEdit ? 'Edit publication resource' : 'Create publication resource'),
      content: Form(
        key: _form,
        child: SizedBox(
          width: 480,
