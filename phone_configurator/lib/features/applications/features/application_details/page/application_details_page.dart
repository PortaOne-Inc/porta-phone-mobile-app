...
class _ApplicationDetailsPageState extends State<ApplicationDetailsPage> with MixinMessages {
  ...
  Widget build(BuildContext context) {
    ...
    return AuthReloginTrigger(
      ...
      child: BlocConsumer<ApplicationDetailsCubit, ApplicationDetailsState>(
                          ...
                          child: ConditionalProgressBar(
                            ...
                            child: Padding(
                              ...
                              child: ApplicationThemesScreen(
                                ...
                                onCopyToApplication: (theme) => _onCopyToApplication(context, theme),
                                onChangeStatus: bloc.changeThemeStatus,
                                onShare: (theme) => _onShareTheme(context, theme),
                              ),
                            ),
                          ),
                        ),
  ...
  Future<void> _onCopyToApplication(BuildContext context, ThemeModel theme) async {
    ...
    try {
      ...
      messenger.showSnackBar(SnackBar(content: Text('Failed to load applications: $e')));
      return;
    }
    ...
    try {
      ...
      messenger.showSnackBar(SnackBar(content: Text('Theme copied to ${result.targetApplicationName}')));
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _copyToAppLoading = false;
      ...
      messenger.showSnackBar(SnackBar(content: Text('Failed to copy theme: $e')));
    }
  }
