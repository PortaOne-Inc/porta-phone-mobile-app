...
Main isolate            ──attach──►       │
                                          └─ SignalingModule (created by app factory)
```
...
The plugin does **not** own or depend on `signaling` in the platform packages.
...
|----------|---------|-------|
| Android | `signaling_service_android` | Foreground service + IsolateNameServer hub; factory resolved in background isolate |
| iOS | `signaling_service_ios` | Main-isolate; factory called directly in `start()` |
...
- `signaling` — `Request` hierarchy, `Event`, `StateHandshake`
- `signaling_service_platform_interface` — `SignalingServicePlatform`, `SignalingModuleEvent`, `SignalingServiceConfig`, `SignalingModuleFactory`
- `signaling_service_android` / `signaling_service_ios` — platform implementations (resolved at build time)
