...
object StorageDelegate {
    private const val PREFS_NAME = "signaling_service"
    ...
    fun saveTrustedCertificatesJson(context: Context, json: String?) =
        prefs(context).edit().putString(KEY_TRUSTED_CERTIFICATES, json).apply()

    fun getTrustedCertificatesJson(context: Context): String? =
        prefs(context).getString(KEY_TRUSTED_CERTIFICATES, null)
    ...
    fun clearConnectionConfig(context: Context) {
        prefs(context).edit()
            ...
            .remove(KEY_TOKEN)
            .remove(KEY_TRUSTED_CERTIFICATES)
            .apply()
    }
}
