...
Wire-level reference for the audio-conference messages this package carries
(WT-783 server side, WT-1911 mobile client): the exact JSON of every request,
response and event, the Dart class for each, what Core does on receipt, what it
refuses with, in which order things arrive, and what the client must do locally at
each step. Enough to write a client against without reading Core.
...
Web Dialer implementation (Gerrit `porta-phone/web-app` 5946b5c). Where the client
guide in `webtrit_core/docs/WT-783-CONFERENCE-CLIENT-API.md` differs from the code,
this document follows the code and says so in section 12.
...
The guide (`webtrit_core/docs/WT-783-CONFERENCE-CLIENT-API.md`) is right on the
model and the local rules; these four points follow the code instead:
...
- Far ends get no participant list and no indication a conference exists; after a
  transfer the remaining party's identity may be wrong (WT-1550 / WT-1160).
