...
/// The builder behind `theme_schema:schema_tables`.
...
class _ClassInfo {
  const _ClassInfo(
    ...
    required this.asksForSchema,
    required this.exposesSchema,
    required this.redirectsTo,
    required this.freezedUnionKey,
    required this.collectionDefaults,
  ...
  /// Whether the annotation carries `createJsonSchema: true`.
  final bool asksForSchema;

  /// Whether the class declares a `jsonSchema` member, without which the
  /// generated constant is unreachable outside its own library.
  final bool exposesSchema;

  /// The class an unnamed redirecting factory points at, if there is one.
  final String? redirectsTo;

  /// The `unionKey` of a `@Freezed(...)` annotation, if the class carries one.
  final String? freezedUnionKey;

  /// Constructor parameters whose default is a non-empty collection literal,
  /// mapped to that literal.
  final Map<String, String> collectionDefaults;

  /// Field name to the converter annotation on it.
  final Map<String, String> converters;
}
...
/// One rule of the contract, broken somewhere.
class ContractViolation {
  const ContractViolation(this.rule, this.where, this.detail);

  /// Short name of the rule, for grouping a report.
  final String rule;

  /// `Type` or `Type.field` - what breaks it.
  final String where;
  ...
  @override
  String toString() => '[$rule] $where: $detail';
}

/// Converters the contract is allowed to keep.
///
/// A converter makes a field's Dart type differ from what the JSON carries, and
/// the schema describes the Dart type - so a converted field is published as
/// something no document contains. `UriConverter` is the one that earns it: a
/// `Uri` is a string on the wire and is described as one.
const _allowedConverters = {'UriConverter'};

/// Every rule of the contract that [sources] break.
///
/// The schema this package publishes is generated by a tool that reads fields off
/// the source class and knows nothing about unions, enums or converters. Five
/// shapes make it describe the contract wrongly, or stop describing it at all, and
/// each is easy to reintroduce by writing an ordinary-looking DTO. Rather than
/// asking anybody to remember them, they are read back out of the source here and
/// `test/contract_rules_test.dart` fails by name.
///
/// [cliSource] is `bin/print_json_schema.dart`, so a new root that nothing can
/// print is caught too. Omit it to skip that one rule.
List<ContractViolation> contractViolations(Map<String, String> sources, {String? cliSource}) {
  final parsed = _parse(sources);
  final byName = {for (final info in parsed.classes) info.name: info};
  final violations = <ContractViolation>[];

  for (final info in parsed.classes) {
    for (final entry in info.collectionDefaults.entries) {
      violations.add(
        ContractViolation(
          'collection-default',
          '${info.name}.${entry.key}',
          'defaults to ${entry.value}. A non-empty collection default crashes schema generation for '
              'the whole library. Take an empty collection and resolve the default when it is read.',
        ),
      );
    }

    for (final entry in info.converters.entries) {
      if (_allowedConverters.contains(entry.value)) continue;
      violations.add(
        ContractViolation(
          'converter',
          '${info.name}.${entry.key}',
          'is behind ${entry.value}. A converter makes the published type differ from what the JSON '
              'carries. Give the field the type the wire has.',
        ),
      );
    }

    if (info.freezedUnionKey != null) {
      violations.add(
        ContractViolation(
          'freezed-union',
          info.name,
          "is a freezed union (unionKey: '${info.freezedUnionKey}'). Its variants are classes freezed "
              'writes, which declare no fields the generator can read. Write a sealed base with an '
              'ordinary class per variant.',
        ),
      );
    }

    if (info.isJsonSerializable && info.fields.isEmpty && info.redirectsTo != null) {
      violations.add(
        ContractViolation(
          'factory-shape',
          info.name,
          'holds its state on ${info.redirectsTo}, so it declares no fields of its own and is published '
              'as an empty object. Declare the fields on the class.',
        ),
      );
    }

    if (info.asksForSchema && !info.exposesSchema) {
      violations.add(
        ContractViolation(
          'unreachable-schema',
          info.name,
          'asks for createJsonSchema but declares no jsonSchema member. The generated constant is '
              'private to its library, so nothing outside the file can read it.',
        ),
      );
    }

    if (info.asksForSchema && info.exposesSchema && cliSource != null) {
      // A variant is reached through its union's table; a root is reached only
      // through the CLI, so a root missing from it is a schema nobody can print.
      final base = byName[info.superName];
      final isVariant = base != null && base.isSealed;
      if (!isVariant && !cliSource.contains('${info.name}.jsonSchema')) {
        violations.add(
          ContractViolation(
            'unprintable-root',
            info.name,
            'exposes a schema but is not in the _roots of bin/print_json_schema.dart, so the CLI '
                'cannot print it.',
          ),
        );
      }
  ...
  violations.sort((a, b) => '${a.rule}${a.where}'.compareTo('${b.rule}${b.where}'));
  return violations;
}
...
_ClassInfo _classInfo(ClassDeclaration declaration, String path) {
  ...
  final collectionDefaults = <String, String>{};
  String? redirectsTo;
  for (final member in declaration.members) {
    if (member is! ConstructorDeclaration || member.name != null) continue;
    redirectsTo ??= member.redirectedConstructor?.type.name.lexeme;
    for (final parameter in member.parameters.parameters) {
      ...
      defaults[name] = value.toSource();
      if (_isNonEmptyCollection(value)) collectionDefaults[name] = value.toSource();
    }
  }

  final fields = <String, _FieldInfo>{};
  final converters = <String, String>{};
  var exposesSchema = false;
  for (final member in declaration.members) {
    if (member is! FieldDeclaration) continue;
    if (member.isStatic) {
      if (member.fields.variables.any((variable) => variable.name.lexeme == 'jsonSchema')) exposesSchema = true;
      continue;
    }
    final type = member.fields.type;
    if (type == null) continue;
    final converter = member.metadata
        .map((entry) => entry.name.name)
        .firstWhere((name) => name.endsWith('Converter'), orElse: () => '');
    ...
    for (final variable in member.fields.variables) {
      ...
      final jsonName = _stringArgument(member.metadata, 'JsonKey', named: 'name') ?? name;
      ...
      if (converter.isNotEmpty) converters[jsonName] = converter;
    }
  }

  final serializable = declaration.metadata.where((entry) => entry.name.name == 'JsonSerializable');
  final freezed = declaration.metadata.where((entry) => entry.name.name == 'Freezed');
  ...
  return _ClassInfo(
    ...
    path,
    fields,
    isJsonSerializable: serializable.isNotEmpty,
    asksForSchema: _boolArgument(declaration.metadata, 'JsonSerializable', 'createJsonSchema') == true,
    exposesSchema: exposesSchema,
    redirectsTo: redirectsTo,
    freezedUnionKey: freezed.isEmpty
        ? null
        : (_namedArgument(declaration.metadata, 'Freezed', 'unionKey') as SimpleStringLiteral?)?.value,
    collectionDefaults: collectionDefaults,
    converters: converters,
  );
}

/// Whether [value] is a collection literal with something in it.
///
/// The empty ones are fine; a non-empty one is what the schema generator hands
/// to a literal writer that has no case for the analyser's own objects.
bool _isNonEmptyCollection(Expression value) {
  if (value is ListLiteral) return value.elements.isNotEmpty;
  if (value is SetOrMapLiteral) return value.elements.isNotEmpty;
  return false;
}
