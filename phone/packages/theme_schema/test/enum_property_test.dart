...
import 'package:theme_schema/schema/enum_properties.g.dart';
...
///
/// The table is written by `lib/builder.dart`, so a new enum
      ...
      for (final root in published.entries) {
        ...
        enumProperties.forEach((type, properties) {
          ...
          properties.forEach((name, property) {
            ...
            // A nullable property carries `null` as one more member - see
            // `assembleNullability` - so the words are compared without it.
            final stated = (described['enum']! as List<Object?>).whereType<String>().toList();
            expect(stated, property.values, reason: '$type.$name does not state its values');
          });
        });
      }
