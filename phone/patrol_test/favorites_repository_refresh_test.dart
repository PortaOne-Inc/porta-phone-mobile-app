...
import '../test/repository/favorites_repository_integration_harness.dart';
...
void main() {
  patrolTest('favorites pull backs off after 503 and persists recovery on the device', ($) async {
    ...
    harness.respond = (_) async {
      ...
      return started.length <= 2
          ...
          : started.length == 3
          ? FavoritesRepositoryIntegrationHarness.successResponse()
    ...
    expect(
      (await harness.local.watchAllWithContacts().first).single.favorite,
      FavoritesRepositoryIntegrationHarness.cached,
    );
    ...
    await waitUntil(
      ...
      description: 'the second favorites pull did not fail',
    );
    ...
    expect(
      (await harness.local.watchAllWithContacts().first).single.favorite,
      FavoritesRepositoryIntegrationHarness.updated,
    );
    ...
    await waitUntil(
      ...
      description: 'favorites recovery did not restore the base interval',
    );
    task.unregister();
  ...
  patrolTest('a saved favorite survives failed push and polling drains its native outbox', ($) async {
    final harness = await _createHarness();
    ...
    await harness.repository.addByContact(
    ...
    expect((await harness.local.watchAllWithContacts().first).single.favorite.number, '1001');
    ...
    harness.respond = (_) async {
      ...
      return FavoritesRepositoryIntegrationHarness.successResponse();
    };
    await _waitForPhase($, task, PollingTaskPhase.succeeded);
    task.unregister();
...
Future<FavoritesRepositoryIntegrationHarness> _createHarness() async {
  final directory = await Directory.systemTemp.createTemp('favorites-refresh-');
  addTearDown(() => directory.delete(recursive: true));
  final harness = FavoritesRepositoryIntegrationHarness(NativeDatabase(File('${directory.path}/favorites.sqlite')));
  addTearDown(harness.dispose);
  await harness.initialize();
  return harness;
}

PollingTaskHandle _register(FavoritesRepositoryIntegrationHarness harness) {
  harness.connectivity.setConnected(true);
...
Future<void> _waitForPhase(PatrolIntegrationTester $, PollingTaskHandle task, PollingTaskPhase phase) => waitUntil(
  ...
  timeout: const Duration(seconds: 10),
  description: 'the favorites task did not reach $phase',
);
