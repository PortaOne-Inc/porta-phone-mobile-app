...
void main() {
  ...
  setUpAll(() {
    ...
    registerFallbackValue(_cachedUser);
  });

  group('UserInfoSyncWorker', () {
    late _MockUserLocalDatasource local;
    late _MockUserRemoteDatasource remote;
    late UserRepository repository;
    late UserInfoSyncWorker worker;
    ...
    test('refresh waits for persistence before publishing and completing', () async {
      ...
      var completed = false;
      final refresh = worker.refresh().then((_) => completed = true);
      await pumpEventQueue();
    ...
    for (final failureSource in ['remote', 'persistence']) {
      ...
      test('$failureSource failures engage polling backoff and success restores the base interval', () {
        fakeAsync((async) {
          ...
          final task = polling.register(PollingRegistration(listener: worker, interval: const Duration(seconds: 10)));
    ...
    test('refresh after disposal is rejected before fetching', () async {
      ...
      verifyNever(() => remote.getInfo());
    });

    test('disposal during a fetch fails the cycle without persisting', () async {
      final fetched = Completer<UserInfo>();
      when(() => remote.getInfo()).thenAnswer((_) => fetched.future);

      final refresh = worker.refresh();
      await pumpEventQueue();
      await worker.dispose();
      fetched.complete(_updatedUser);
