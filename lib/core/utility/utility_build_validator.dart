class UtilityBuildValidator {
  ...
  final _platformIdentifierRegex = RegExp(r'^[a-zA-Z]+[a-zA-Z0-9_]*(\.[a-zA-Z]+[a-zA-Z0-9_]*)*$');
  final _urlRegex = RegExp(r'^(https?|ftp)://[^\s/$.?#].\S*$');

  bool isBuildVersionNameValid(String? name) => _versionValidator.isValidVersionName(name ?? '');

  bool isBuildVersionNumberValid(int? number) => _versionValidator.isValidBuildNumber(number?.toString() ?? '');

  bool isPlatformIdentifierValid(String? packageName) => _platformIdentifierRegex.hasMatch(packageName ?? '');
...
class VersionValidator {
  ...
  static final RegExp extendedVersionNameRegExp = RegExp(r'^\d+\.\d+\.\d+\+\d{6}[a-z]{4}$');
  ...
  bool isValidVersionName(String versionName) {
    ...
    if (a < 0 || b < 0 || c < 0 || abc < 0 || int.tryParse(xxxx) == null || int.parse(xxxx) >= 1000) {
      return false;
    }
