...
class SettingsMainScreenTab extends StatelessWidget {
  const SettingsMainScreenTab({required this.config, required this.onChanged, super.key});
    ...
    return ListView(
      ...
      children: [
        ...
        AppBarSurfaceEditor(
          ...
          onAppBarBlurredSurfaceChanged: (v) => onChanged(config.copyWith(appBarBlurredSurface: v)),
        ),
        const SizedBox(height: 16),
        BorderContainer(
          title: 'Layout & Structure',
          descriptionWidget: DescriptionRow.info('Configure the structural appearance of the main settings list.'),
          child: SeparatorStyleEditor(
            value: config.separator,
            onChanged: (separator) => onChanged(config.copyWith(separator: separator)),
          ),
        ),
        const SizedBox(height: 16),
        BorderContainer(
          title: 'List Items Styling',
          descriptionWidget: DescriptionRow.info('Customize typography and iconography for individual setting rows.'),
          padding: const EdgeInsets.all(16),
          child: Column(
            ...
            children: [
              ColorInput(
                ...
                onTap: () => _pickColor(context, groupConfig.backgroundColor?.toColor(), (hex) {
                  onChanged(config.copyWith(groupTitleListTile: groupConfig.copyWith(backgroundColor: hex)));
                }),
                onClear: () =>
                    onChanged(config.copyWith(groupTitleListTile: groupConfig.copyWith(backgroundColor: null))),
              ),
              const SizedBox(height: 16),
              TextStyleConfigEditor(
                label: 'Header Typography',
                value: groupConfig.textStyle,
                onChanged: (newTextStyle) =>
                    onChanged(config.copyWith(groupTitleListTile: groupConfig.copyWith(textStyle: newTextStyle))),
                onClear: () => onChanged(config.copyWith(groupTitleListTile: groupConfig.copyWith(textStyle: null))),
              ),
            ],
          ),
        ),
